      ...
      {pages.map((page, pageIndex) => {
        return (
          <article key={pageIndex} className="mx-auto flex h-[1120px] w-[min(794px,calc(100vw-48px))] flex-col bg-white px-8 py-7 font-serif text-[14px] leading-relaxed text-slate-900 shadow-sm sm:px-12">
            ...
            <div className={`${pageIndex === 0 ? "h-[780px]" : "h-[920px]"} shrink-0 whitespace-pre-wrap py-8 overflow-hidden`}>
              {page.fragments.map((fragment) => <div key={`${fragment.blockIndex}-${fragment.start}`} className="mb-3 break-words" dangerouslySetInnerHTML={{ __html: fragment.text || "" }} />)}
            </div>
