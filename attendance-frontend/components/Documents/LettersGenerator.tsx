...
export default function LettersGenerator() {
  ...
  const templateLayout = useMemo(() => template ? paginateDynamicTemplateBlocks(splitDynamicTemplateBlocks(template.content), A4_PAGINATION_GEOMETRY) : [], [template]);
  const save = async (send: boolean) => { if (!employee || !template) return toast.error("Select a template and employee"); const preview = previewRef.current; if (!preview || preview.dataset.layoutMeasured !== "true") return toast.error("Letter preview is still preparing. Please try again in a moment."); if (preview.dataset.layoutOverflow === "true") return toast.error("Document content does not fit within the saved template layout."); setSaving(true); try { await api.post("/employee-documents/letters/generate", { template_id: template.id, employee_id: employee.id, send, placeholder_values: resolvedValues, template_layout: templateLayout, layout_validated: true }); toast.success(send ? "Letter sent to employee documents" : "Letter draft generated"); } catch (error) { toast.error(getErrorMessage(error)); } finally { setSaving(false); } };
