...
export default function LettersGenerator() {
  ...
  const resolvedTemplateLayout = useMemo(() => template ? paginateDynamicTemplateBlocks(splitDynamicTemplateBlocks(resolved), A4_PAGINATION_GEOMETRY) : [], [template, resolved]);
  const save = async (send: boolean) => { if (!employee || !template) return toast.error("Select a template and employee"); const preview = previewRef.current; if (!preview || preview.dataset.layoutMeasured !== "true") return toast.error("Letter preview is still preparing. Please try again in a moment."); if (preview.dataset.layoutOverflow === "true") return toast.error("Document content does not fit within the saved template page layout."); setSaving(true); try { await api.post("/employee-documents/letters/generate", { template_id: template.id, employee_id: employee.id, send, placeholder_values: resolvedValues, template_layout: resolvedTemplateLayout, layout_validated: true }); toast.success(send ? "Letter sent to employee documents" : "Letter draft generated"); } catch (error) { toast.error(getErrorMessage(error)); } finally { setSaving(false); } };
  return <div className="space-y-6"><section className="rounded-xl border border-ink-200 bg-white p-5 shadow-card"><div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">Generate Letter</h2><p className="mt-1 text-sm text-ink-500">Select a database template and employee, then preview, download, or send.</p></div><FilePlus2 className="text-brand-600"/></div><label className="mt-5 block max-w-md text-sm font-medium">Select Template<select value={templateId} onChange={e => { setTemplateId(e.target.value); setEmployeeId(""); setPlaceholderValues({}); }} className="mt-1 block w-full rounded-lg border-ink-200"><option value="">Select template</option>{templates.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></section>{template && <><section className="rounded-xl border border-ink-200 bg-white p-5 shadow-card"><label className="block max-w-md text-sm font-medium">Select Employee<select value={employeeId} onChange={e => { setEmployeeId(e.target.value); setPlaceholderValues({}); }} className="mt-1 block w-full rounded-lg border-ink-200"><option value="">Select employee</option>{employees.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></section>{employee && <div className="grid gap-6 lg:grid-cols-[minmax(0,20fr)_minmax(0,80fr)] lg:items-start"><section className="rounded-xl border border-ink-200 bg-white p-4 shadow-card"><h3 className="font-semibold">Letter Details</h3><p className="mt-1 text-sm text-ink-500">Review and edit values before sending.</p><div className="mt-4 space-y-3">{placeholderKeys(template.content).map(key => <label key={key} className="block text-sm font-medium text-ink-700">{placeholderLabel(key)}<input value={placeholderValues[key] ?? employeeValues[key] ?? ""} onChange={e => setPlaceholderValues(current => ({ ...current, [key]: e.target.value }))} className="mt-1 w-full rounded-lg border border-ink-200 px-3 py-2 text-sm" /></label>)}</div></section><div className="min-w-0 rounded-xl border border-ink-200 bg-ink-100 p-3 sm:p-6"><div className="mb-3 flex flex-wrap justify-between gap-2"><p className="text-xs font-medium text-ink-500">PREVIEW</p><div className="flex w-full flex-wrap gap-2 sm:w-auto sm:justify-end"><button onClick={() => void downloadDynamicLetterPdf(template.name, resolved, employee.name, previewRef.current)} className="inline-flex min-h-10 flex-1 items-center justify-center gap-1 rounded-lg border bg-white px-3 py-2 text-sm sm:flex-none"><Download size={14}/> Download PDF</button><button onClick={() => save(false)} disabled={saving} className="inline-flex min-h-10 flex-1 items-center justify-center gap-2 rounded-lg border bg-white px-3 py-2 text-sm sm:flex-none"><Eye size={14}/> Generate Draft</button><button onClick={() => save(true)} disabled={saving} className="inline-flex min-h-10 flex-1 items-center justify-center gap-2 rounded-lg bg-brand-600 px-3 py-2 text-sm text-white sm:flex-none"><Send size={14}/> Send</button></div></div><div className="overflow-auto"><DynamicLetterPreview ref={previewRef} title={template.name} content={resolved} templateContent={template.content} /></div></div></div>}</>}</div>;
}
