...
export default function FeedbackPage() {
  ...
  const query = (feedbackType: FeedbackType, page: number) => ({ feedback_type: feedbackType, visibility: visibility || undefined, year: month ? Number(month.slice(0, 4)) : undefined, month: month ? Number(month.slice(5, 7)) : undefined, start_date: startDate || undefined, end_date: endDate ? `${endDate}T23:59:59` : undefined, search: search || undefined, sort, page, page_size: PAGE_SIZE });
  const load = async () => {
    if (!admin) return;
    try {
      const [positiveResponse, negativeResponse] = await Promise.all([
        api.get<FeedbackResponse>("/feedback/", { params: query("positive", positivePage) }),
        api.get<FeedbackResponse>("/feedback/", { params: query("negative", negativePage) }),
      ]);
  ...
  useEffect(() => { load(); }, [admin, visibility, month, startDate, endDate, search, sort, positivePage, negativePage]);
