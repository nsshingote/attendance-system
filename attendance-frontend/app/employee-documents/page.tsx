...
const latestSentSlipFor = (slips: Slip[], employeeId: string) => slips
  .filter(slip => slip.employee_id === Number(employeeId) && slip.status === "Sent")
  .sort((first, second) => new Date(second.sent_at || second.created_at || 0).getTime() - new Date(first.sent_at || first.created_at || 0).getTime())[0];
  ...
  return <AppShell allowedRoles={["admin", "superadmin"]}><div className="mx-auto max-w-6xl space-y-6">
    ...
    {tab === "Letters" ? (
      ...
      <section className="rounded-xl border border-ink-200 bg-white p-4 shadow-card sm:p-6"><h2 className="text-lg font-semibold">Generate Salary Slip</h2><p className="mb-5 text-sm text-ink-500">Create and send salary slips to employees</p>
        <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium">Select Employee<select value={employeeId} onChange={e => { setEmployeeId(e.target.value); setEditingSlipId(null); setRows(rowsFromSlip(latestSentSlipFor(slips, e.target.value))); }} className="mt-1 block w-full rounded-lg border-ink-200"><option value="">Select employee</option>{employees.map(employee => <option key={employee.id} value={employee.id}>{employee.name}</option>)}</select></label><label className="text-sm font-medium">Month &amp; Year<input type="month" value={period} onChange={e => setPeriod(e.target.value)} className="mt-1 block w-full rounded-lg border-ink-200" /></label></div>
