...
export default function AttendancePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [recordsRes, calendarRes, summaryRes] = await Promise.all([
        api.get<AttendanceRecord[]>(attendanceUrl, { params }),
        api.get<CalendarDay[]>("/attendance/calendar", { params: { year, month, user_id: userId === -1 ? undefined : userId } }),
        api.get(`/attendance/summary/${userId === -1 ? session?.userId : userId || session?.userId}`, { params: { year, month } }).catch(() => ({ data: {} })),
      ]);

      // Records now include has_report from backend
      const recordsWithReport = (recordsRes.data || []).map((record: any) => ({
        ...record,
        report: typeof record.has_report === "boolean"
          ? (record.has_report ? "Submitted" : "Not Submitted")
