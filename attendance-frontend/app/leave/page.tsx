...
export default function LeavePage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      if (admin && effectiveEmployeeIds.length === 0) {
        ...
        const leaveResults = await Promise.all(userLeavePromises);
  ...
  const isViewingSpecificUsers = (admin || teamView) && selectedUserIds.length > 0;
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
              Requests submitted at least 4 days before the leave starts can use Paid leave. Short-notice requests use Carried, then Unpaid. Company holidays are not counted as leave days.
            </p>
          </div>
