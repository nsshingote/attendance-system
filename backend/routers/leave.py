...
# def _has_paid_leave_this_month(db: Session, user_id: int) -> bool:
#     """Check if user has any Pending or Approved Paid leave this month."""
#     today = date.today()
...
#         and_(
#             LeaveRequest.from_date >= start_of_month,
...
# def _get_paid_leave_available(db: Session, user_id: int) -> int:
#     """Get paid leave available this month (0 or 1)."""
#     user = db.query(User).filter(User.id == user_id).first()
#     if not user:
#         return 0
    
#     if _has_paid_leave_this_month(db, user_id):
#         return 0
    
#     return user.paid_leave_available or 0


# def _get_carried_leave_balance(db: Session, user_id: int) -> int:
#     """Get carried leave balance for a user."""
#     user = db.query(User).filter(User.id == user_id).first()
#     if not user:
#         return 0
...
#         if _has_paid_leave_this_month(db, target_user_id):
#             raise HTTPException(
#                 status_code=400,
#                 detail="You have already used your paid leave for this month"
#             )
#         if _get_paid_leave_available(db, target_user_id) <= 0:
#             raise HTTPException(
#                 status_code=400,
#                 detail="No paid leave available for this month"
#             )
    
#     elif payload.leave_category == "Carried":
#         carried_balance = _get_carried_leave_balance(db, target_user_id)
...
#         if target_user.carried_leave < total_days:
#             raise HTTPException(
#                 status_code=400,
#                 detail=f"Insufficient carried leave balance. Available: {target_user.carried_leave}"
#             )
...
def apply_leave(
    ...
    if payload.leave_category == "Paid":
     if not paid_leave_available_this_month(db, target_user):
        raise HTTPException(
            status_code=400,
            detail="No paid leave available for this month"
        )
